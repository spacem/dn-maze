...
module.exports.fn = new (
  function() {
    ...
    this.url = {
      ...
      image: function(file) {
        return uri.image + (file ? '/' + timestamp + file + '.png' : '');
      }
    }
    ...
    this.image = {
      ...
      skilltree: function(job, version, region) {
        if(region == 'sea' || region == 'eu' || region == 'na' || region == 'th') {
          return 'background-image:url(' + thiz.url.skill('c-awake/' + job.EnglishName) + ')';
        }
        else if(region == 'tw' || region == 'kdn' || region == 'cdn') {
          return 'background-image:url(' + thiz.url.skill('kdn/' + job.EnglishName) + ')';
        }
        else {
